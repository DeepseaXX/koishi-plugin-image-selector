...
用于在聊天中快速创建新分类文件夹，或为已有分类增加别名。这两条指令**不会**出现在全局帮助列表中。

**语法：**
- `创建关键词 <文件夹名> [别名1] [别名2]...` （指令名可由 `createCommandName` 自定义）
- `创建别名 <已有关键词> <新别名1> [新别名2]...` （指令名可由 `addAliasCommandName` 自定义）

**关键机制：**
- **专属权限**：仅当用户 ID 存在于配置的 `createLimits` 列表中时才允许使用，无权限者调用会被直接忽略。
- **冲突保护**：执行前会自动进行全局查重。若发现要创建的分类名、或新添加的别名已被现有图库占用，将自动放弃操作，防止覆盖和冲突。
