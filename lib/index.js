...
function apply(ctx, config) {
  ...
  ctx.command(`${config.saveCommandName} [角色名称] [...图片]`, { captureQuote: false }).userFields(["id", "name", "authority"]).action(async ({ session }, 角色名称, ...图片) => {
    if (session.quote) {
    ...
    try {
      ...
      const resultMessage = 角色名称 ? `已保存 ${savedCount} 个文件到"${角色名称}"文件夹` : `已保存 ${savedCount} 个文件到临时文件夹`;
